...
MODEL_PATH = os.getenv("YELO_MODEL_PATH", "models/yolo_garbage.pt")
# Weights live in the repository's models/ directory. Resolve them regardless
# of the directory the gateway is launched from, and accept either a bare file
# name ("yolo_garbage.pt") or a models-relative path ("models/yolo_garbage.pt").
if not os.path.isabs(MODEL_PATH) and not os.path.exists(MODEL_PATH):
    ...
    for _candidate in (
        ...
        os.path.join(_repo_root, "models", os.path.basename(MODEL_PATH)),
    ):
        if os.path.exists(_candidate):
            MODEL_PATH = os.path.normpath(_candidate)
            break
