    ...
    return (
      <>
        ...
        <section className="panel token-result" aria-labelledby="token-title">
          ...
          <div className="form-actions"><Link href={`/cameras/view?id=${cameraId}`} className="secondary-button focus-ring">Open camera</Link><Link href={`/capture?returnTo=${encodeURIComponent(`/cameras/view?id=${cameraId}`)}`} className="primary-button focus-ring">Connect device <Camera size={18} /></Link></div>
        </section>
      </>
    );
  }
